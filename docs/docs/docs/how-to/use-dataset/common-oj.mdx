...
export const generateDocFunc = ({selectedDataset, config, generatePythonDict}) => {
  config.dataset_type = 'CommonOJDataset';
...
# avoid large test transfer
config['provided_data'] = [{**d, 'test': ''} for d in data]
...
<DatasetSelector datasetKey="CommonOJ" generateDocFunc={generateDocFunc} />
