...
def extract_code_from_freeform_completion(completion, sample, language, first_block_only: bool = False):
    LANGUAGE_TO_ALIASES = {
        "python": ["python", "Python", "py", "Python3", "python3", "PY"],
        "java": ["java", "Java"],
    ...
    def extract_sql_code(text):
        """从原始completion中，抽出code实现"""
        code = "\n".join([line for line in text.split("\n") if line.strip() != ""]) + "\n"

        pattern_sql = "^\s*(?:select|with\s[^\n]+as)[^;]*"
        matches = re.findall(pattern_sql, code, re.M | re.IGNORECASE)
        return matches
    ...
    # filter out all the code blocks with non-target language mark
    if language:
        code_blocks = [m[1] for m in code_matches if m[0].strip() in LANGUAGE_TO_ALIASES[language] + [""]]
        code_langs = [m[0].strip() for m in code_matches if m[0].strip() in LANGUAGE_TO_ALIASES[language] + [""]]
    else:
        # Fallback, which the case `language` argument is empty.
        code_blocks = [m[1] for m in code_matches]
        code_langs = [m[0].strip() for m in code_matches]

    # Normalize `code_langs`.
    code_langs = [ALIASES_TO_LANGUAGE_TILED.get(c, "") for c in code_langs]
    language = language or ""  # normalize `None` to ''

    # For fewshot case, only extract the first coding block.
    if sample.get("is_fewshot_task") is True:
        first_sp_block_idx = next((i for i, lang in enumerate(code_langs) if lang == language), -1)
        first_un_block_idx = next((i for i, lang in enumerate(code_langs) if lang == ""), -1)
        # Extract first bash block for non-bash language,
        # because some pre-operation such `pip install` here.
        # Consider use it in the future.
        first_sh_block_idx = next((i for i, lang in enumerate(code_langs) if lang == "bash"), -1)
        ...
        if first_block_idx != -1:
            ...
            code_langs = code_langs[:first_block_idx + 1]
        logging.debug("[select_first_code_block_for_fewshot_task]\n{}".format(code_blocks))

    # Extract code which is not surrounded by markdown fenced code block.
    if len(code_blocks) == 0:
        if language == "python":
            code_blocks.extend(extract_py_code(completion))
        elif language == "sql":
            code_blocks.extend(extract_sql_code(completion))
        elif language == "bash":
            code_blocks.extend(extract_common_code(completion))

    if first_block_only:
        if code_blocks:
            completion = code_blocks[0].replace("\r", "")
        else:
            completion = ""
    else:
        completion = "\n\n".join(code_blocks).replace("\r", "")
    ...
    if language == "python":
        ...
        if "class Solution" not in completion:
            for line in completion.split("\n"):
                if sample["entry_point"] in line:
                    completion = completion.replace(line, "class Solution {\n" + line)
                    completion += "\n}"
                    break
        # Add import statements
        for line in sample["declaration"].split("\n"):
            if "import" in line:
                completion = line + "\n" + completion
    elif language == "go":
        # Remove package main
